...
class Melee(commands.Cog):
  ...
  async def melee(self, ctx):
    ...
    if arg_count != self.args:
      ...
      if in_list:
        async with aiohttp.ClientSession() as session:
          async with session.get('https://survivio.fandom.com/wiki/' + act_weapon) as r:
            unparsed = await r.read()
        ...
        embed = discord.Embed(title=f'{act_weapon} Stats', description=f'**Damage**: {damage} \n **Attack Radius**: {rad} \n **Equip Speed**: {equip_speed} \n **Cooldown Time**: {cltime} \n **Auto Attack**: {auto}', color=0x00b037)
        embed= embed = discord.Embed(title=f'{act_weapon} Stats', description=f'**Damage**: {damage} \n **Attack Radius**: {rad} \n **Equip Speed**: {equip_speed} \n **Cooldown Time**: {cltime} \n **Auto Attack**: {auto} \n', color=0x00b037)
        ...
        for i in wep_dict.keys():
          if big_concat == '':
            ...
            place = ', ' + i
            big_concat += place
            embed = discord.Embed(description=f'**"{weapon}"** is not a valid weapon in **surviv.io**. \n \n  **Valid Weapons**: ' + big_concat, color=0x00b037)
        await ctx.send(embed=embed)
def setup(bot):
