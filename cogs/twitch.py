        ...
        async with aiohttp.ClientSession() as session:
          async with session.get(url=self.url, headers=self.headers) as r:
            if r.status == 200:
              ...
              main = b
              embed = discord.Embed(title="Current Twitch Streamers", description=main, color=0x00b037)
              await ctx.send(embed=embed)
            else:
              await ctx.send('Bad Web Request to the surviv.io API. Log an issue with the issue command')
