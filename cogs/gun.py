      ...
      async with aiohttp.ClientSession() as session:
        async with session.get(self.url) as r:
          try:
            ...
            ctx.send('Bad Web Request to the surviv.io API. Log on issue with the **issue command**.')
          unparsed =  await r.read()
      try:
        b = soupify(unparsed, 'html.parser').find_all('table', {'class': 'article-table'})[1].find_all('tr')
        print(b)
      except:
        await ctx.send("Looks like the program couldn't find what you were looking for. Please **log an issue description** with the **issue command**.")
