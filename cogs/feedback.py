...
class Feedback(commands.Cog):
  ...
  async def issue(self, ctx):
    ...
    if arg_count == 0:
      await ctx.send(f'**{name}** command only takes an argument count of **{len(args)}** \n{msg}')
    else:
      issue = ' '.join(args[1:])
      conn2 = await aiosqlite.connect('issues.db')
      c2 = await conn2.cursor()
      await c2.execute('''create table if not exists issues(
      ...
      d = await c2.fetchall()
      d = d[0][0]
      embed = discord.Embed(description = f"**Issue #{d}** Registered: '**{issue}**'", color=0x00b037)
  ...
  async def sug(self, ctx):
    ...
    if arg_count == 0:
      ...
      c2 = await conn2.cursor()
      await c2.execute('''create table if not exists suggestions(
                    id integer primary key autoincrement,
                    suggestion text
                    )''')
      await conn2.commit()
      await c2.execute('insert into suggestions (suggestion) values (?)', [suggestion])
      await conn2.commit()
      await c2.execute('SELECT MAX(ID) FROM SUGGESTIONS')
      d = await c2.fetchall()
      d = d[0][0]
      embed = discord.Embed(description = f"**Suggestion #{d}** Registered: '**{suggestion}**'", color=0x00b037)
      await ctx.send(embed=embed)
    ...
    if author in self.admin:
      ...
      for r in b:
        if concat != '':
          ...
          concat += f'{r[1]}'
        embed = discord.Embed(title='**Suggestions**', description=f'{concat}', color=0x00b037)
        user = self.bot.get_user(ctx.message.author.id)
      await user.send(embed=embed, delete_after=3600)
    else:
      await ctx.send("You checked the code repo. Good for you, but you can't run this command unless you contribute to the bot")
    ...
    if author in self.admin:
      ...
      for r in b:
        if concat != '':
          ...
          concat += f'{r[1]}'
        embed = discord.Embed(title='**Issues**', description=f'{concat}', color=0x00b037)
        user = self.bot.get_user(ctx.message.author.id)
      await user.send(embed=embed, delete_after=3600)
    else:
      await ctx.send("You checked the code repo. Good for you, but you can't run this command unless you contribute to the bot")
